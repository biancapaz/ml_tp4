...
def train_AE(model, train_loader, test_loader, device, epochs=50, lr=1e-3):
    """Entrena un autoencoder con perdida MSE"""
...
def reconstruct_AE(model, X, device):
    model.eval()
    with torch.no_grad():
        if hasattr(X, "to_numpy"):
            X = X.to_numpy()
